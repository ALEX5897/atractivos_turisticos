...
async function runMigration() {
  ...
  try {
    const migrationPath = path.join(__dirname, '../database/migration_12_add_fields_to_rutas.sql')
    ...
    console.log('🔄 Ejecutando migración 12: Agregar campos a tabla rutas...')
    ...
    console.log('\n✅ Migración 12 completada exitosamente')

    // Verificar que los campos se crearon
    const [columns] = await conn.query(`DESCRIBE rutas`)
    const newFields = ['codigo_qt', 'nodo', 'centralidad', 'clasificacion', 'modalidad', 'link_de_ruta', 'establecimiento_a_b', 'observacion_de_inactivacion']
    const createdFields = columns.filter(col => newFields.includes(col.Field)).map(col => col.Field)

    console.log(`\n✅ Campos creados: ${createdFields.length}/${newFields.length}`)
    createdFields.forEach(field => console.log(`  ✓ ${field}`))

    await conn.end()
    process.exit(0)
  } catch (err) {
    console.error('\n❌ Error ejecutando migración 12:', err.message)
    if (err.sql) {
